        ...
        def modify_mask(self):
            ...
            for i_idx in range(8):
                for j_idx in range(8):
                    mean = xp.mean((xp.sum(mask[:, :, i_idx * 8:i_idx * 8 + 8, j_idx * 8: j_idx * 8 + 8])))
                    mask[:, :, i_idx * 8:i_idx * 8 + 8, j_idx * 8: j_idx * 8 + 8] = mean
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total-iteration", "-iter", type=int, default=500)
    parser.add_argument("-ckpt", type=str, default="./mask_imgs")
